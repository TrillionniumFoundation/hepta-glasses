...
class ExternalEvidenceSigningTransactionTest(unittest.TestCase):
    ...
    def test_private_key_symbolic_link_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(ValueError, "must reference a regular file"):
    ...
    def test_custody_root_symbolic_link_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.skipTest(f"symbolic links are unavailable: {error}")
            with self.assertRaisesRegex(ValueError, "non-directory or symbolic-link component"):
                signing_io.create_scoped_uri_exclusive(alias, "artifact://signature.bin", b"signature", 1024, label="linked custody output")
    ...
    def test_replaced_custody_root_fails_before_output(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            def transaction() -> None:
                ...
                root.rename(retired)
                root.mkdir()
                signing_io.create_scoped_uri_exclusive(root, "artifact://signature.bin", b"signature", 1024, label="replacement custody output")

            with self.assertRaisesRegex(ValueError, "directory object changed during the transaction"):
                transaction()
    ...
    def test_replaced_existing_output_parent_fails_before_output(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            @core.validation_snapshot
            def transaction() -> None:
                core._validate_lexical_directory(parent, label="initial output parent")
                parent.rename(retired)
                parent.mkdir()
                signing_io.create_scoped_uri_exclusive(root, "artifact://outputs/signature.bin", b"signature", 1024, label="replacement output parent")

            with self.assertRaisesRegex(EvidenceError, "directory object changed during the transaction"):
                transaction()
    ...
    def test_signature_and_successor_share_one_parent_generation(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            @core.validation_snapshot
            def transaction() -> None:
                signing_io.create_scoped_uri_exclusive(root, "artifact://outputs/signature.bin", b"signature", 1024, label="detached signature")
                parent.rename(retired)
                parent.mkdir()
                signing_io.create_scoped_uri_exclusive(root, "artifact://outputs/successor.json", b"{}\n", 1024, label="successor bundle")
    ...
    def test_high_level_authority_operations_are_snapshot_wrapped(self) -> None:
        for function in (signing.sign_submission, signing.sign_reviewer, signing.finalize):
