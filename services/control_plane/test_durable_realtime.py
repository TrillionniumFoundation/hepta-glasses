...
class Tests(unittest.TestCase):
    def setUp(self): self.t=tempfile.TemporaryDirectory(); self.p=Provider(); self.s=DurableRealtimeStore(self.t.name+"/r.db",provider=self.p)
    def tearDown(self): self.s.close(); self.t.cleanup()
    def test_ticket_single_use_and_generation(self):
        ticket=self.s.issue_ticket(subject="u",session_id="s",now=1); row=self.s.activate(ticket=ticket,subject="u",session_id="s",now=2)
        with self.assertRaisesRegex(DurableRealtimeError,"replayed"): self.s.activate(ticket=ticket,subject="u",session_id="s",now=2)
    ...
    def test_indeterminate_reconciles_without_second_activation(self):
        ticket=self.s.issue_ticket(subject="u",session_id="s",now=1); self.p.fail=True
        with self.assertRaisesRegex(DurableRealtimeError,"indeterminate"): self.s.activate(ticket=ticket,subject="u",session_id="s",now=2)
