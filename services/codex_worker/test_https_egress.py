...
class HttpsEgressTests(unittest.TestCase):
    ...
    def exchange(self, response: bytes, *, url="https://api.example.com/v1?q=1", method="GET", body=b"", delay=0):
        ...
        with patch("services.codex_worker.https_egress._resolve_addresses",
                   return_value=(_Address(socket.AF_INET,"1.1.1.1"),)), \
             patch("services.codex_worker.https_egress._connect_tls", return_value=client) as connect:
            try:
    ...
    def test_get_uses_exact_host_and_one_http_request(self):
        result, observed, connect = self.exchange(b"HTTP/1.1 200 OK\r\nContent-Length: 2\r\n\r\nok")
    ...
    def test_post_body_and_fixed_headers(self):
        result, observed, _ = self.exchange(b"HTTP/1.1 201 Created\r\nContent-Length: 0\r\n\r\n",
    ...
    def test_url_and_method_boundaries(self):
        for url in ("http://api.example.com/", "https://user@api.example.com/",
                    "https://api.example.com:444/", "https://API.EXAMPLE.COM/",
                    "https://api.example.com/#fragment", "https://other.example.com/"):
            self.error("egress_url_invalid" if url.startswith("http:") or "user@" in url or ":444" in url or "#" in url else "egress_domain_denied",
                       lambda u=url:self.broker.request(u))
        self.error("egress_method_denied", lambda:self.broker.request("https://api.example.com/",method="PUT"))
        self.error("egress_request_invalid", lambda:self.broker.request("https://api.example.com/",body=b"x"))

    def test_target_rejects_crlf_backslash_space_and_noncanonical_percent(self):
        for suffix in ("/%0dX", "/%0AX", "/bad\\path", "/bad path", "/%2f", "/%G0"):
            self.error("egress_target_invalid", lambda s=suffix:self.broker.request("https://api.example.com"+s))

    def test_policy_rejects_wildcards_ip_literals_duplicates_and_unsorted(self):
        for domains in (("*.example.com",),("1.1.1.1",),("b.example.com","a.example.com"),("a.example.com","a.example.com")):
            self.error("egress_domains_invalid", lambda d=domains:EgressPolicy(d))

    def test_private_or_mixed_dns_is_rejected(self):
        for rows in ([[int(socket.AF_INET),"127.0.0.1"]],
                     [[int(socket.AF_INET),"1.1.1.1"],[int(socket.AF_INET),"10.0.0.1"]],
                     [[int(socket.AF_INET6),"fe80::1"]]):
            self.error("egress_dns_address_rejected", lambda r=rows:_validate_addresses(r))
    ...
    def test_truncated_and_slow_responses_fail_closed(self):
        ...
        original = self.broker; self.broker = short
        try:
    ...
    def test_policy_and_response_headers_are_immutable(self):
        ...
        result,_,_=self.exchange(b"HTTP/1.1 200 OK\r\nH-Test: yes\r\nContent-Length: 0\r\n\r\n")
        with self.assertRaises(TypeError): result.headers["x-test"]="no"
