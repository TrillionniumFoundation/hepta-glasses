...
import 'package:hepta_glasses/runtime/policy_engine.dart';
import 'package:hepta_glasses/runtime/tool_gateway.dart';
...
void main() {
  ...
  test(
    'terminal receipt survives gateway restart without duplicate effect',
    () async {
      final clock = MutableClock(now);
      final journal = InMemoryAuditJournal(clock: clock);
      var effects = 0;
  ...
  test(
    'indeterminate effect is cached and resolved only by reconciler',
    () async {
      final clock = MutableClock(now);
      final journal = InMemoryAuditJournal(clock: clock);
      final gateway = ToolGateway(
